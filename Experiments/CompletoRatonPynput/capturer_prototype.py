...
from pynput import mouse #El raton se captura con pynput
...
ejes mando: axes[0], axes[1], axes[2], axes[3], axes[4], axes[5]
16 botones
...
def on_click(x, y, button, pressed):
    if button == button.left:
        mouse_buttons[0] = int(pressed) #pressed es bool, 1 para True, 0 para False
    elif button == button.right:
        mouse_buttons[2] = int(pressed)
    elif button == button.middle:
        mouse_buttons[1] = int(pressed)
        print(f"Boton: {button}")
    #print('{0} at {1}'.format('Pressed' if pressed else 'Released',(x, y)))

mouse_listener = mouse.Listener(
    on_move=on_move,
    on_click=on_click,
    #on_scroll=on_scroll #No disponible todavia
    )
...
while running:
    ...
    #print(teclas_packet)
    packet = struct.pack(packet_format,*teclas_packet,*axes,*buttons,*dpad,*mouse_buttons, *mouse_pos)
    sock.sendto(packet, (SERVER_IP, PORT))
    #print(f"Paquete: {struct.unpack(packet_format, packet)}", end="\r")
    print(f"Teclas: {teclas_packet}\nEjes: {axes}\nBotones: {buttons}\nDpad: {dpad}\nRaton: {mouse_buttons}\nRaton_pos: {mouse_pos}", end="\r")
    #print(*dpad)
